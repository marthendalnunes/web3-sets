...
import {
  RuleOperation,
  RuleOperationBeforeTimestampArgs,
...
/**
 * @name ruleOperationBeforeTimestamp
 * @description Check if a condition has occurred before a certain timestamp
 ...
 * @throws If the operation method is not 'beforeTimestamp'
 * @throws If the operation arguments are invalid
 * @throws If the reference is invalid
 */
export function ruleOperationBeforeTimestamp(
  ...
  if (method !== 'beforeTimestamp') throw new Error('Invalid operation method')

  const safeArgs = RuleOperationBeforeTimestampArgs.safeParse(args)
  ...
  const references: Array<RuleOperationResultsRef | undefined> = refs.map(
    (ref) => {
      if (ref.startsWith('condition:')) {
        ...
        // Check if all the operations have a timestamp smaller than the timestamp passed in

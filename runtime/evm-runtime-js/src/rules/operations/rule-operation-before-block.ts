...
import { RuleOperation, RuleOperationBeforeBlockArgs } from 'src/types/set/rule'
import {
...
/**
 * @name ruleOperationBeforeBlock
 * @description Check if a condition has occurred before a certain block
 ...
 * @throws If the operation method is not 'beforeBlock'
 * @throws If the operation arguments are invalid
 * @throws If the reference is invalid
 */
export function ruleOperationBeforeBlock(
  ...
  if (method !== 'beforeBlock') throw new Error('Invalid operation method')

  const safeArgs = RuleOperationBeforeBlockArgs.safeParse(args)
  ...
  const references: Array<RuleOperationResultsRef | undefined> = refs.map(
    (ref) => {
      if (ref.startsWith('condition:')) {
        ...
        // Check if all the operations have a blockNumber smaller than the blockNumber passed in
